...
> **Дедлайн артефакта 1**: 25.09.2026 (приёмка 11.09.2026)
...
| 0:10–1:10 | Приёмка артефакта 1 | Основная работа: приём по чек-листу |
| 1:10–1:20 | (опц.) Разбор архитектуры СМП + карта требований | Резиновый блок, дропается при затянувшейся приёмке |
| 1:20–1:30 | Q&A + ДЗ | План до дедлайна 25.09, анонс модуля 2 |
...
**Что на экране:**
- Статус потока: список Issue'ов с label `practice-1`, CI-статусы репозиториев студентов;
...
- Дедлайн Артефакта 1 — **25.09.2026** (приёмка сегодня, 11.09.2026).

**Скрипт для преподавателя:**

> На прошлой практике вы запускали СМП и гоняли smoke. Сегодня — проходная: я и 1–2 ассистента параллельно принимаем Артефакт 1 по чек-листу. Правила простые: либо у вас в репозитории зелёный CI, либо мы локально прогоняем smoke по чек-листу. Дедлайн артефакта 1 — 25 сентября: сегодня приёмка, до дедлайна — время на доработку без штрафа. У кого не заработало на прошлой практике — сегодня доделываем, все вопросы — в общий документ, ассистенты отвечают там же.

---

## Часть 2. Приёмка артефакта 1 (0:10–1:10)

### Как организована приёмка

- Ведущий + 1–2 ассистента принимают **параллельно** по единому чек-листу.
- Критерий сдачи: **CI зелёный** в репозитории студента **или** **локальный прогон smoke** по чек-листу ниже.
...
### Правило приоритета приёмки

1. **CI зелёный** → отметка «сдано», время: 30 секунд на студента
2. **CI красный / нет CI** → локальный прогон smoke по чек-листу, время: 3–5 минут на студента
3. **Docker не запускается** → фиксация проблемы, доработка до дедлайна 25.09

> При 30 студентах: 20 с зелёным CI (10 мин) + 10 с локальным прогоном (50 мин) = 60 минут. Укладываемся.

> **Что значит «зелёный CI»:** прошли ОБЕ проверки — лёгкая CI (автоматически на push) и тяжёлый smoke (запускается вручную: Actions → Practice Run (heavy checks) → `1`). Один зелёный лёгкий чек не гарантирует, что smoke реально отработал. Источник: [`practice-01.md`](practice-01.md), раздел «Что проверит CI».
...
### 3.1. Разбор архитектуры (1:10–1:15)
...
### 3.2. Карта требований (1:15–1:20)
...
**Шаблон карты требований (3 строки для совместной работы):**
...
**Домашнее задание:** продолжить карту ещё на 2–3 строки (резервирование, генерация карт, срок действия).

---

## Часть 4. Q&A + ДЗ (1:20–1:30)

**Что на экране:** план до дедлайна 25.09; анонс модуля 2.

**Подготовка к практике 3 (модуль 2, 18.09) — тест-дизайн:**

1. **Дочитать ТЗ** [`04-authorization.md`](https://github.com/LektonSoftwareTestingCourse/Practic/blob/main/tz/04-authorization.md) и [`05-card-management.md`](https://github.com/LektonSoftwareTestingCourse/Practic/blob/main/tz/05-card-management.md) — фокус на правилах авторизации и резервирования
2. **Продолжить карту требований** из Части 3.2 ещё на 2–3 строки: резервирование, генерация карт, срок действия (`expiryDate`)
3. **(Опционально)** Если не сделано на практике 1 — сформулировать 3–5 пользовательских сценариев: успешная транзакция + отказы по разным причинам

> **Критерий готовности к практике 3:** карта требований ≥ 5 строк, покрывающая статусы, лимиты, баланс и срок действия. Это заготовка для артефакта 2 — тест-дизайна, который начнётся 18 сентября с Ольгой Лихачевой.
...
2. **1–2 ассистента**, знающих чек-лист. **Fallback:** при отсутствии ассистентов приёмка только по CI (зелёный = сдано), локальный прогон — по записи в общий документ проблем с разбором после занятия.
