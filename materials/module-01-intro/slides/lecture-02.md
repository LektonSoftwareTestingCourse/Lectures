...
|:---:|---|:---:|---|
| 0:00–0:10 | Ввод и повторение | 1–2 | Связь с лекцией 1 и практикой 1, план |
| 0:10–0:32 | Жизненный цикл ПО (SDLC) | 3–6 | Модели, фазы, где тестирование |
| 0:32–0:52 | Процесс тестирования (STLC) | 7–9 | Фазы, артефакты, роли |
| 0:52–1:05 | Релизная политика и тестирование | 10–12 | Почему релиз за час — не всегда цель, реальность Lekton |
| 1:05–1:30 | Практика и артефакт | 13–15 | Модель данных, decision table, анонс |
...
> Продолжаем первый модуль. На прошлой лекции (03.09) мы разобрались, что такое тестирование, развенчали основные мифы — помните testing vs checking и «fake testing»? — и познакомились с учебным проектом СМП. На прошлой неделе (04.09) была практика 1 — содержательная: запускали СМП, гоняли smoke-тесты, разбирались с Docker. Сегодня поговорим о том, как тестирование встроено в процесс разработки. А завтра (11.09) — практика 2: проходная, приём артефакта 1. Если у кого-то Docker так и не запустился — завтра разберёмся, дедлайн артефакта 1 — 18 сентября.
...
- Релизная политика и подход к тестированию: почему нельзя всегда релизить за час
...
- ISTQB: «Тестирование присутствует в каждой модели жизненного цикла, но с очень разными значениями» [Spillner et al., 2014, 2.2]
...
> Качество нельзя «проверить в конце» — оно строится на каждом этапе. Это, кстати, один из семи принципов ISTQB, которые мы обсуждали на прошлой лекции: «Раннее тестирование» — начинайте тестировать как можно раньше.
...
> **Waterfall** — каскадная модель [Royce, 1970]. Фазы идут строго последовательно: завершили требования → проектирование → разработка → и только потом тестирование. Тестирование здесь — «финальная приёмка», последняя проверка перед сдачей [Spillner et al., 2014, 2.2]. Ключевой недостаток: дефекты, заложенные в требованиях, обнаруживаются слишком поздно — стоимость их исправления удваивается на каждом пройденном уровне тестирования [Spillner et al., 2014, 6.3.1]. В чистом виде Waterfall сегодня редкость, но его элементы встречаются в регулируемых отраслях.
...
> **Требования.** Тестировщик анализирует спецификации. Можно ли это вообще протестировать? Нет ли противоречий между разделами? Какие приёмочные критерии? ISTQB называет это «анализ основы для тестирования» — проверка требований как фундамента тестов [Spillner et al., 2014, 2.2.2]. Если требование сформулировано как «система должна быть быстрой» — это нетестируемо. Нужно: «время ответа API не должно превышать 200 мс для 95-го перцентиля при 500 одновременных запросах».
...
> **Проектирование.** Тестировщик участвует в ревью архитектуры. Не будет ли узких мест? Как изолировать компоненты для тестирования? Это называется «анализ тестируемости» — оценка, насколько систему можно будет протестировать. Составляется план тестирования и оцениваются риски.
...
> Ключевой принцип, о котором мы говорили на прошлой лекции — **независимость тестировщика** [Bach & Bolton, 2025, Ch.1]. «Самый сильный аргумент в пользу выделенных тестировщиков — это отсутствие конфликта интересов. Поскольку они НЕ отвечают за создание проблем и НЕ отвечают за их исправление, они могут искать и сообщать о проблемах без страха и сожалений.» Разработчик, написавший код, психологически не может глубоко тестировать свою работу — срабатывает эффект подтверждения [Spillner et al., 2014, 2.3]. Поэтому нужен выделенный тестировщик.

> Причём независимость нужна не только организационно, но и **когнитивно** — эти роли мыслят по-разному. Разработчик мыслит *конвергентно*: сводит хаос требований к работающей реализации, ищет единственно правильное решение. Для него требование — это контракт: сделал по спецификации — задача выполнена. Тестировщик мыслит *дивергентно*: выходит за рамки спецификации в контекст бизнеса и пользователя, генерирует множество сценариев, включая «невозможные». Для него требование — не контракт, а гипотеза, которую нужно проверить на стыке с реальным миром. Пример: требование «поле ввода даты рождения». Разработчик думает про DatePicker и формат ДД.ММ.ГГГГ. Тестировщик — про 29 февраля, американский формат ММ/ДД и возрастное ограничение 18+. Именно на этом построен подход **Shift-Left** — «сдвиг тестирования влево» по жизненному циклу: привить «тестировочное» мышление разработчикам ещё на этапе кодирования, а не ждать отдельной фазы тестирования. К инструментам Shift-Left мы вернёмся в модуле 8, когда будем собирать CI/CD-пайплайн.
...
### Слайд 10. Релизная политика и подход к тестированию

**Слайд:**
- Релизная политика определяет риск → риск определяет подход к тестированию
- Прямой деплой на прод (startup/SaaS): можно релизить часто, ловить баги в проде, быстрые хотфиксы
- Вендорская модель (enterprise): релиз уходит клиенту на установку — нужна гарантия качества ДО отгрузки
...
> Вопрос с практики: «Почему нельзя выпустить релиз системы за час, автоматизировав все проверки?» Ответ — в релизной политике компании.

> **Два полюса.** Первая модель: вы деплоите напрямую на свой прод. Один прод, одна конфигурация, вы контролируете инфраструктуру. Сборка за 10 минут, smoke-тесты, canary-релиз — и можно жить с циклом в час. Баг? Хотфикс за полчаса. Пользователи простят, если быстро починить.

> Вторая модель: вы — вендор. Пишете софт, который устанавливается у клиента. У вас не один прод, а десять. У каждого своя ОС, своя СУБД, свои бизнес-настройки. Кто-то на Linux с PostgreSQL, кто-то на Windows с Oracle. Кто-то мигрирует с rpm-пакетов на k8s. Вы не можете «откатить» чужой прод — клиент сам решает, когда ставить обновление, и сам тестирует у себя. Разрыв между вашей сборкой и установкой у клиента — недели.

> **Матрица тестирования.** Вендор тестирует не «систему», а комбинацию: 3 ОС × 2 СУБД × 5 конфигураций × 2 линейки = 60 вариантов. Тесты нельзя распараллелить полностью: часть кейсов требует последовательного выполнения из-за shared-ресурсов. Бэкпорт из мастера в 25.2 — это отдельный цикл тестирования на старой кодовой базе. Точечный релиз для одного клиента — ещё один цикл.

> Humble и Farley в *Continuous Delivery* [Ch.10] прямо пишут: «Выпустить новую версию приложения в production-окружение, которое вы контролируете — это одно. Выпустить новую версию ПО, которое пользователи устанавливают на свои машины — это совсем другое». Для клиент-устанавливаемого ПО они рекомендуют тестировать upgrade на репрезентативной выборке целевых окружений. В Lekton это не выборка — это вся матрица.

> Поэтому enterprise-тестирование — это не «потому что enterprise медленный», а потому что цена ошибки на чужом проде выше, а возможность быстро исправить — ниже. В Lekton полный E2E-цикл на трёх линейках занимает одну–две недели. Это не бюрократия — это инженерная необходимость.

> [ИНТЕРАКТИВ] Посчитайте сами: если у вас 3 клиента, 2 ОС и 2 версии в поддержке — сколько вариантов тестирования? А если 10 клиентов и 3 линейки? Умножьте на среднее время E2E-прогона. Поймёте, почему «релиз за час» — это привилегия прямого деплоя, а не универсальная цель.

> **Связь с deep/shallow testing.** На прошлой лекции мы говорили о deep testing в enterprise и shallow testing в стартапе. Релизная политика — это механизм, который делает этот выбор неизбежным. Прямой деплой позволяет shallow testing: можно быстро откатить, можно отследить ошибку в мониторинге, можно починить за час. Вендорская модель требует deep testing: баг уедет к клиенту, и вы не сможете его быстро починить. Поэтому вендор инвестирует в тестирование ДО релиза, а не ПОСЛЕ.
...
**Слайд:**
- Enterprise = гибрид: V-модель для регуляторики + Agile для фич
- CI/CD на всех контурах, ~40 тестировщиков, Java-стек
- Ключевой вывод: тестирование встроено в каждый этап, а не «в конце»

**Speaker notes:**

> Кратко — мы уже разбирали Lekton на прошлой лекции (слайд 8). Сегодня фокус: enterprise = гибридная модель. Регуляторные требования (PCI DSS, ЦБ) — по V-модели с полной документацией. Новые фичи — по Agile. Это инженерное решение, а не религия.
...
> Семь ключевых принципов архитектуры СМП — и каждый из них порождает тестовые сценарии. Разберем еще раз и посмотрим, что будем тестировать.
...
> Подведём итог по практикам модуля. Практика 1 (04.09) — содержательная: запускали СМП, гоняли smoke, разбирались с Docker. Это Артефакт 1, 6 баллов, проверяется CI.
>
> Практика 2 (11.09) — **проходная**. Что это значит? На содержательной практике вы получаете новый материал и работаете над артефактом. На проходной — я и ассистенты принимаем уже сделанный артефакт по чек-листу. Минимум теории, максимум проверки и Q&A. Если у вас зелёный CI — сдача занимает 2 минуты. Если нет — локально прогоняем smoke и разбираемся, что не так.
>
> Дедлайн артефакта 1 — 18 сентября. Если завтра не успеете сдать — не страшно, дорабатываете до дедлайна без потери баллов. Но лучше сдать завтра и получить обратную связь.
...
3. **Humble, J., & Farley, D.** (2010). *Continuous Delivery: Reliable Software Releases through Build, Test, and Deployment Automation*. Addison-Wesley. ISBN 978-0-321-60191-9. — Глава 10 (Deploying and Releasing Applications — различие между деплоем на свой прод и поставкой клиенту).

4. **ISTQB** — International Software Testing Qualifications Board. *Certified Tester Foundation Level Syllabus*. [https://www.istqb.org](https://www.istqb.org)
