...
> **Обновлять при:** добавлении модулей, смене структуры каталогов, изменении кросс-репозиторных связей.
...
flowchart TD
    subgraph GOV[Слой управления курсом]
        ...
        RPD[admin/РПД две версии]
        DOC[admin/DocsForItmoStudents/TestingFor.md]
        REV[admin/review-module-02-open-questions.md]
    end
...
контент, кроме review-документа, который адресно ревьюит модуль 2.
...
`SlidevSlides` содержит слайды (по review-документу — модуль 2), то есть часть
контента физически находится вне `Lectures`. `testing-course-internal` содержит
...
| `materials/admin/` | РПД, FAQ, ревью, описание для ИТМО | Учебный контент |
